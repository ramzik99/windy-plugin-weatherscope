<section class="winter-workspace" aria-label="Wintry Forecast integrated workspace">
 <div class="winter-heading"><div><h2>Wintry Forecast</h2><p>ECMWF profiles · Windy terrain · up to 144 hours</p></div><span class="snowflake" aria-hidden="true">❄</span></div>
 <p class="source-note">Winter diagnostics use ECMWF, independently of the general forecast baseline. All views follow the same selected point and Windy time. Winter times use this device's timezone.</p>
 <div class="winter-tools"><button class:enabled={contoursEnabled} aria-pressed={contoursEnabled} on:click={toggleContours}>{contoursEnabled?'Hide':'Show'} snowline contours</button><button disabled={probeLoading||!location} on:click={refreshForecast}>Refresh winter data</button></div>
 {#if contoursEnabled}<p class="map-legend">╱ Terrain above estimated snowline · contours −500 to 6000 m</p>{/if}
 {#if viewportLoading}<p role="status">Updating snowline contours…</p>{/if}
 {#if refreshError}<p role="status">{refreshError}</p>{/if}
 <div class="winter-point" style={`--winter-accent:${pointAccent}`}><small>AT THE SELECTED TIME · ECMWF</small><h3>{pointTitle}</h3><div class="winter-detail">{@html pointDetail}</div></div>
 {#if clickedPoint}<div class="winter-time"><label>Explore winter forecast<input aria-label="Winter forecast time" type="range" min="0" max={clickedPoint.times.length-1} value={selectedSlot} on:input={e=>setTime(Number(e.currentTarget.value))}/></label><button on:click={()=>setTime(nearestIndex(clickedPoint.times,Date.now()))}>Now</button></div>
 <SnowlineChart embedded point={clickedPoint} terrainM={clickedMapElevationM} placeName={clickedPlaceName||placeName||'Selected point'} bind:tab={forecastTab} units={unitSystem}/>
 {#if clickedNextEventTime!==null}<button class="copy-winter" on:click={()=>store.set('timestamp',clickedNextEventTime)}>Go to next wintry interval →</button>{/if}
 <button class="copy-winter" on:click={e=>shareCurrentPoint(e.currentTarget)}>Copy winter summary</button>{/if}
 <details><summary>How to interpret winter guidance</summary><p>The snowline is a wet-bulb-zero proxy, not proof of snowfall. Precipitation type is profile-derived. New snow is a forecast estimate, not observed snow depth. Low confidence and unavailable inputs remain visible. Contours sample the visible map and may miss small terrain features.</p></details>
</section>
<script lang="ts">
  ...
  let mounted=false,contoursEnabled=false,pointTitle='Select a location',pointDetail='',pointAccent='#78e4ca',selectedInput='',selectedSlot=0;
  $: if(mounted){unitSystem=units;updatePersistentClickLabel();if(contoursEnabled)renderFromCache();}
  $: if(mounted&&location&&selectedInput!==location.lat+','+location.lon){selectedInput=location.lat+','+location.lon;void probeLocation(location.lat,location.lon,'map-click',placeName||null);}
  $: if(placeName&&clickedLatLon)clickedPlaceName=placeName;
  function toggleContours(){contoursEnabled=!contoursEnabled;if(contoursEnabled)void refreshViewport();else{generation++;viewportLoading=false;refreshQueued=false;clearContours();}}
  function setTime(index:number){const t=clickedPoint?.times[index];if(Number.isFinite(t))store.set('timestamp',t);}
  function handlePointAction(e:MouseEvent|KeyboardEvent){if((e.target as HTMLElement)?.closest('.snowline-event-jump')&&clickedNextEventTime!==null)store.set('timestamp',clickedNextEventTime);}
  const syncTimestamp=()=>{if(contoursEnabled&&cache.length&&!viewportLoading)renderFromCache();if(clickedPoint){selectedSlot=Math.max(0,forecastIntervalIndex(clickedPoint.times,getStoreTimestamp()));updatePersistentClickLabel();}};
  ...
  async function refreshViewport(){
    if(!enabled||!contoursEnabled||destroyed)return;
  ...
  function showClickLabel(lat:number,lon:number,mainText:string,detailHtml='',snowlineColor='#fff',status:ProbeStatus='neutral'){pointTitle=mainText;pointDetail=detailHtml;pointAccent=statusColor(status);}
  ...
  async function probeLocation(lat:number,lon:number,source:PointSource,placeName:string|null=null){
    ...
    try{
      ...
      if(keep)chartOpen=true;selectedSlot=Math.max(0,forecastIntervalIndex(point.times,getStoreTimestamp()));updatePersistentClickLabel();
  ...
  onMount(()=>{mounted=true;unitSystem=units;register(config.name,'high');freshnessTimer=setInterval(checkAutomaticRefresh,30_000);document.addEventListener('visibilitychange',checkAutomaticRefresh);window.addEventListener('online',checkAutomaticRefresh);map.on('moveend',handleMapNavigation);map.on('zoomend',handleMapNavigation);store.on('timestamp',syncTimestamp);});
  onDestroy(()=>{mounted=false;destroyed=true;release(config.name,'high');refreshEpoch++;generation++;clickGeneration++;pendingProfiles.clear();if(freshnessTimer)clearInterval(freshnessTimer);document.removeEventListener('visibilitychange',checkAutomaticRefresh);window.removeEventListener('online',checkAutomaticRefresh);if(moveTimer)clearTimeout(moveTimer);map.off('moveend',handleMapNavigation);map.off('zoomend',handleMapNavigation);store.off('timestamp',syncTimestamp);clearContours();clearClickLayer();profileCache.clear();});
...
<style lang="less">
 ...
 .winter-workspace{color:#edf4f8;font:13px/1.5 system-ui}.winter-heading{display:flex;align-items:center;justify-content:space-between}.winter-heading h2{font-size:22px;color:#edf4f8;margin:0}.winter-heading p,.source-note{color:#aec1ca;font-size:11px}.snowflake{font-size:35px;color:#8ed9fa}.winter-tools{display:flex;gap:8px;margin:14px 0;flex-wrap:wrap}.winter-tools button,.winter-time button,.copy-winter{padding:9px 12px;border:1px solid #34515d;border-radius:9px;background:#17323e;color:#e2f0f1;cursor:pointer;min-height:40px}.winter-tools .enabled{border-color:#78e4ca;color:#78e4ca}.winter-point{padding:18px;border:1px solid #35505c;border-top:3px solid var(--winter-accent);border-radius:14px;background:linear-gradient(130deg,#13343d,#182537);margin:15px 0}.winter-point>small{font-size:10px;color:#adc4cf}.winter-point h3{color:var(--winter-accent);font-size:24px;margin:10px 0}.winter-time{display:flex;align-items:center;gap:12px;margin:16px 0}.winter-time label{flex:1;font-size:12px;color:#aec1ca}.winter-time input{display:block;width:100%;accent-color:#78e4ca;min-height:32px}.copy-winter{margin-top:14px}.map-legend{font-size:11px;color:#d0afef}details{margin:16px 0;color:#aec1ca;font-size:12px}summary{cursor:pointer}.winter-detail :global(.snowline-label-grid){gap:8px}.winter-detail :global(.snowline-label-grid span){padding:10px}.winter-detail :global(.snowline-label-grid strong){font-size:17px}.winter-detail :global(.snowline-label-grid small){font-size:10px}.winter-detail :global(.snowline-event-line){font-size:12px}.winter-detail :global(.snowline-valid){font-size:12px;margin-bottom:10px}

 .winter-workspace{color:var(--ink,#edf7fa)}.winter-heading h2{font-weight:500}.winter-heading p,.source-note{font-size:12px;color:var(--muted,#b2c7d1)}.winter-point{background:var(--aurora-hero,linear-gradient(125deg,#193e43,#183044));border-radius:16px;border-color:var(--line,#36505e);border-top-color:var(--winter-accent)}.winter-point>small{font-size:11px}.winter-point h3{font-weight:500}.winter-tools button,.winter-time button,.copy-winter{background:var(--panel,#152b37);border-color:var(--line,#36505e);min-height:44px;border-radius:10px}.winter-tools .enabled{color:var(--mint,#7be4ca);border-color:var(--mint,#7be4ca)}.winter-time input{accent-color:var(--mint,#7be4ca)}.winter-detail :global(.snowline-label-grid small){font-size:11px}.winter-detail :global(.snowline-label-grid strong){font-weight:500}.winter-detail :global(.snowline-label-grid span){background:#152b37;border-radius:12px;padding:12px}
