<div class="chart-shell" class:embedded role={embedded?'region':'dialog'} aria-modal="false" aria-label="Wintry forecast graph" bind:this={chartShell} style={embedded?'':`left:${position.x}px;top:${position.y}px;transform:none;`}>
  <div class="chart-head">
    ...
    <div class="chart-actions">
      ...
      {#if !embedded}<button class="drag-button" type="button" title="Drag graph" aria-label="Drag graph" on:pointerdown={startDrag}>↕</button>
      <button type="button" title="Close" aria-label="Close graph" on:click={() => dispatch('close')}>×</button>{/if}
    </div>
  </div>
...
<script lang="ts">
  ...
  const onTimestamp=(v:any)=>{const n=Number(v);if(Number.isFinite(n))timestamp=n;};
  ...
  onMount(() => {
    ...
    try { const t = store.get('timestamp'); if (typeof t === 'number') timestamp = t; store.on('timestamp',onTimestamp); } catch {}
  });
  onDestroy(() => { if (realNowTimer) clearInterval(realNowTimer); window.removeEventListener('pointermove', dragMove); window.removeEventListener('pointerup', stopDrag); try { store.off('timestamp',onTimestamp); } catch {} });
</script>
...
.chart-shell.embedded{position:relative;inset:auto;z-index:0;width:100%;max-width:none;max-height:none;overflow:visible;padding:12px;box-shadow:none;box-sizing:border-box}.embedded .chart-title small,.embedded .chart-title em{font-size:11px}.embedded .forecast-tabs button{min-height:38px}.embedded .event-head b{font-size:13px}

 .chart-shell.embedded{background:var(--panel,#152b37);border:1px solid var(--line,#36505e);border-radius:16px;padding:16px;color:var(--ink,#edf7fa)}.embedded .chart-head{background:var(--panel,#152b37)}.embedded .chart-title b{font-weight:500;font-size:17px}.embedded .chart-actions button{min-height:40px;min-width:40px;font-weight:500}.embedded .forecast-tabs button{font-size:12px;min-height:44px;font-weight:500}.embedded .forecast-tabs button.active{background:#28524f;color:#d9fff5;box-shadow:none}.embedded .metrics small,.embedded .event-timeline small,.embedded .elevation-grid small{font-size:11px}.embedded .metrics b,.embedded .event-timeline span,.embedded .elevation-grid b{font-size:12px;font-weight:500}.embedded .current-type b{font-size:15px;font-weight:500}.embedded .current-type strong,.embedded .outlook24>b,.embedded .outlook24 span,.embedded .outlook24 button{font-size:11px}.embedded .metrics{gap:8px}.embedded .metrics span{padding:10px 5px}.embedded .event-head b{font-weight:500}.embedded .confidence{font-size:11px}.embedded .elevation-head b{font-size:13px}.embedded .elevation-head span,.embedded .impact-band,.embedded .elevation-grid em{font-size:11px}.embedded .elevation-grid{grid-template-columns:repeat(auto-fit,minmax(75px,1fr));gap:6px}
